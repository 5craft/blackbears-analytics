...
2.	Выполнить команду _composer global require "fxp/composer-asset-plugin:*"_, затем запустить скрипт из папки с корнем проекта _composer_ командой `composer install`
    ...
    Если ваша версия PHP < 5.6 следует применить фикс: из папки _корень-проекта/data/fix_ скопировать файл _Statement.php_ в директорию _корень-проекта/vendor/8bitov/clickhouse-php-client/src/_
...
4.	Выполнить необходимые минимальные настройки в конфигах:
    ...
    В файле _/backend/config/params-local.php_ указать значение параметра _yandex_oauth_id_. [Инструкция по получению __yandex_oauth_id__](#Настройка-Яндекс-oauth-авторизации).

    В файле _/common/config/main-local.php_ указать параметры подключения к ClickHouse:
    ```

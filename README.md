...
4.	Выполнить необходимые минимальные настройки в конфигах:
  1. В файле _/backend/config/params-local.php_ указать значение параметра _yandex_oauth_id_. [Инструкция по получению _yandex_oauth_id_](Page#yandex-oauth). 
...
6. Настроить [отправку событий в AppMerica] (Page#appmetrica-events)
7. Запуск скрипта по [сборку событий из AppMerica] (Page#appmetrica-grab)
...
9. Если требуется подсчет __Ads Revenue__ и в AppMerica передается соответствующее событие, то необходимо [подключить рекламные площадки](Page#ads-platform-connect)

  Установка и настройка завершена.


### Настройка Яндекс oAuth-авторизации [Page#yandex-oauth]
----
...
В "_Callback URL_" указать путь "_http://адрес_вашего_проекта/login?_".
...
### Отправка в AppMerica событий и их структура (Page#appmetrica-events)
----
...
1. Для подсчета __In-App Revenue__ приложения в AppMerica необходимо отправлять событие _purchase_ (вы можете использовать любое название события, не забудьте только указать его в параметрах скрипта для [сбора информации из AppMetrica](Page#appmetrica-grab)) со следующими параметрами:
...
2. Для подсчета __Ads Revenue__ приложения в AppMetrica необходимо отправлять событие _ads_view_ (вы можете использовать любое название события, не забудьте только указать его в параметрах скрипта для [сбора информации из AppMetrica](Page#appmetrica-grab)) со следующими параметрами:
...
### Сбор информации по событиям из AppMetrica [Page#appmetrica-grab]
----
...
### Получение информации Ads Revenue по приложениям ### (Page#ads-platform-connect)
----
...
1. организовать отправку в [AppMetrica событий о просмотре рекламы] (Page#appmetrica-events) 
